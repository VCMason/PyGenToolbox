...
def run_hista2_PE(aligndatabase, forwardfile, reversefile, paired=True):
    import os
    import subprocess
    ...
    if paired == True:
        ...
        cmd = 'hisat2 -q -x %s -1 %s -2 %s -S %s' % (aligndatabase, forwardfile, reversefile, outsamfile)
    else:
        # then data is single end
        print('Starting Hisat2: aligning\n%s' % forwardfile)
        outsamfile = os.path.join(path, 'hisat2', '.'.join(f.split('.')[:-1] + [referenceprefix, 'sam']))
        cmd = 'hisat2 -q -x %s -U %s -S %s' % (aligndatabase, forwardfile, outsamfile)
...
def main(forward, reverse, directory='', cleanreads=False, paired=True,
    ...
    for count, forwardfile in enumerate(forwardlist):
        ...
        samfile = run_hista2_PE(reference, forwardtrim, reversetrim, paired=True)
