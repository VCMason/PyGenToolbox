...
def run_hista2_PE(aligndatabase, forwardfile, reversefile, knownsplicesites, paired=True):
    import os
    import subprocess
    ...
    if paired == True:
        ...
        cmd = 'hisat2 -q --no-softclip --pen-noncansplice 0 --known-splicesite-infile %s -x %s -1 %s -2 %s -S %s' % (knownsplicesites, aligndatabase, forwardfile, reversefile, outsamfile)
    else:
        # then data is single end
        print('Starting Hisat2: aligning\n%s' % forwardfile)
        outsamfile = os.path.join(path, 'hisat2', '.'.join(f.split('.')[:-1] + [referenceprefix, 'sam']))
        cmd = 'hisat2 -q --no-softclip --pen-noncansplice 0 --known-splicesite-infile %s -x %s -U %s -S %s' % (knownsplicesites, aligndatabase, forwardfile, outsamfile)
...
def main(forward, reverse, directory='', cleanreads=False, paired=True,
         reference='/media/sf_LinuxShare/Ciliates/Genomes/Hisat2_Indexes/Pt_51_Mac', knownsplicesites='/media/sf_LinuxShare/Ciliates/Genomes/Annotations/ss.IES.pt_51_MacAndIES.table'):
    ...
    for count, forwardfile in enumerate(forwardlist):
        ...
        if paired == True:
            ...
            samfile = run_hista2_PE(reference, forwardtrim, reversetrim, knownsplicesites, paired=True)
