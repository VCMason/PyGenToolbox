...
def retrieve_rna_sequences_from_contacts_inside_features(seqnames, rnafastqfiles, buffer=0):
    import gzip
    import os
    ...
    print(f'Total Number of RNAs (after pear, before alignment): {len(list(drna.items()))}')
    # print(seqnames[:2])
    print(f'Total Number of Contacts in features: {len(seqnames)}')
    ...
    outrnas = [b'>%s\n%s' % (bytes(k, encoding='utf8'), drna[bytes(k, encoding='utf8')]) for k in seqnames]
    print(f'Number of unique rna sequences from contacts in features: {len(outrnas)}')
    outshortrnas = [b'>%s\n%s' % (bytes(k, encoding='utf8'), drna[bytes(k, encoding='utf8')]) for k in seqnames if len(drna[bytes(k, encoding='utf8')]) <= 30]
    print(f'Number of unique short rna sequences (<=30bps) from contacts in features: {len(outshortrnas)}')
    outscnrnas = [b'>%s\n%s' % (bytes(k, encoding='utf8'), drna[bytes(k, encoding='utf8')]) for k in seqnames if len(drna[bytes(k, encoding='utf8')]) == 25]
    ...
    outfile = os.path.join(path, file.split('split')[0] + f'RNAs.ContactsNearIES.buffer{buffer}.fastq.gz')
    outshortfile = os.path.join(path, file.split('split')[0] + f'RNAs.ContactsNearIES.buffer{buffer}.Short.fastq.gz')
    outscnfile = os.path.join(path, file.split('split')[0] + f'RNAs.ContactsNearIES.buffer{buffer}.25bp.fastq.gz')
...
def limit_contacts_by_feature_with_indices(dgff3, contactfile, side='DNA', buffer=0):
    ...
    # i.e. rnascaffold, rnastart, rnaend, dnascaffold, dnastart, dnaend, contactstrength, readname1,readname2,readname3
    # side == 'DNA' or 'RNA': molecule RNA-bridge-DNA, RNA contact position, or DNA contact position inside feature
    # buffer allows extension of feature coordinates by specified amount start - buffer & end + buffer
    import os
    from natsort import natsorted
    ...
    if side == 'DNA':
        ...
        scaffcolumn = 3  # number use 0-based numbering for columns
        coordinatecolumn = 4
    elif side == 'RNA':
        ...
        print(f'side variable {side} not specified correctly as DNA or RNA')

    with open(contactfile, 'r') as FILE:
        for count, line in enumerate(FILE):  # for each contact in contactfile
            support = int(line.strip().split()[-2])
            scaff = line.strip().split()[scaffcolumn][2:]  # isolate scaffold number only (keep as string)
            start = int(line.strip().split()[coordinatecolumn])
            #dnascaff = 'scaffold51_%d' % int(l[3][2:])
            ...
            try:
                ...
                for k, v in dgff3[scaff].items():  # use gff3 index for scaffold to reduce runtime
                    ...
                    #if (rnascaff == v[0])
                    if (start >= (int(v[3]) - buffer)) and (start <= (int(v[4]) + buffer)):
                        dcounts[k] = dcounts.setdefault(k, 0) + support
    ...
    path, file = os.path.split(contactfile)
    outfile = os.path.join(path, '.'.join(file.split('.')[:-1]) + f'.{side}insidefeatures.buffer{buffer}.txt')
    with open(outfile, 'w') as OUT:
        OUT.write('\n'.join(outlines))
    outfilecounts = os.path.join(path, '.'.join(file.split('.')[:-1]) + f'.{side}insidefeatures.buffer{buffer}.counts.txt')
...
def limit_contacts_by_feature(dgff3, contactfile):
    ...
    with open(contactfile, 'r') as FILE:
        for count, line in enumerate(FILE):
            l = line.strip().split()
            #support = int(math.ceil(float(l[-1])))  # round up to nearest integer # math.ceil returns float()
            rnascaff = 'scaffold51_%d_with_IES' % int(l[0][2:])
            rnastart = int(l[1])
            #dnascaff = 'scaffold51_%d' % int(l[3][2:])
...
def summarize_raw_rna_dna_contacts_and_names(rawcontactfilenames):
    ...
    with open(rawcontactfilenames, 'r') as FILE:
        for l in FILE:  # if memory wasn't a problem i would do this
            # line structure rnascaffold, rnastartpos, rnaendpos, dnascaff, dnastart, dnaend, NNNbarcode, read name
    ...
    with open(outpath, 'w') as OUT:
        # line structure: below, AANNNAA column is omitted with PCRDupRemoval = True
        # rnascaffold rnastartpos rnaendpos dnascaff dnastart dnaend AANNNAA strength
    ...
    with open(outpathcircos, 'w') as OUT:
        ...
        OUT.write('\n'.join([' '.join([i] + ['thickness=' + str(math.log(len(d[i]) + 1, 2)) + 'p']) for i in natsorted(d)]))  # sortcontacts
            ...
            for rnapos in drna[k]:  # the read could map to multiple locations 'equally' well (according to MQ cutoff)
                ...
                for dnapos in ddna[k]:  # the read could map to multiple locations 'equally' well (according to MQ)
                    ...
                    if PCRDupRemoval is False:
                        ...
                        output.append([sntrim + rnapos[0].split('_')[1], rnapos[1], str(int(rnapos[1]) + 1),
                                       sntrim + dnapos[0].split('_')[1], dnapos[1], str(int(dnapos[1]) + 1),
                                       k])
...
def record_read_positions(bamfile, MQ, dreads={}, end='5'):
            ...
            if line[0] == '@':
                ...
                if f'{flag:012b}'[-3] == '0':  # if the read is mapped # '000000000100' == 4 == read unmapped #[-3] != 1
                    if f'{flag:012b}'[-5] == '1':  # if 1 then it is reverse # '000000010000' == 16 read mapped reverse
                        ...
                        if end == '5':  # coord == five prime coordinate
                            coord = get_rightmost_reference_based_alignment_coordinate(
                                line.strip().split('\t')[5],
                                int(line.strip().split('\t')[3]))
                        elif end == '3':  # coord == three prime coordinate
                            coord = line.strip().split('\t')[3]
                        # key is read name, value is scaffold name and then 5' or 3' position of read mapped
                        dreads.setdefault(line.strip().split('\t')[0], []).append(
                            [line.strip().split('\t')[2], str(coord)])
...
def main(forwardreadfile, reversereadfile, blastdatabase, aligndatabase, altreadfile1='/path/to/altreads1.fastq',
         ...
         'fastq2fasta', 'blast', 'hisat2', 'samtools', 'rnadnacontacts', 'heatmap'], rnalen=15, dnalen=15,
         rnamaxlen=1000, REsequence='GATC', windowsize=300, contactcutoff=1, MQ=10, buffer=0, sn='Paramecium tetraurelia', alignargs='',
    ...
    if 'rnadnacontacts' in steps:
        ...
        for count, (rnabam, dnabam) in enumerate(zip(rnasortbamfiles, dnasortbamfiles)):
            print(dnabam)
            drna, ddna = {}, {}
            drna = record_read_positions(rnabam, MQ, drna, end='3')
            ddna = record_read_positions(dnabam, MQ, ddna, end='5')
    ...
    if 'sequences in features' in steps:
        if steps[0] == 'sequences in features':
            contactfile = altreadfile1
        dgff3 = read_gff3_index_by_scaffold(annot_file, features)
        featurecontactfile, dcounts = limit_contacts_by_feature_with_indices(dgff3, contactfile, side='DNA', buffer=buffer)
        ...
        retrieve_rna_sequences_from_contacts_inside_features(seqnames, rnafastqfiles, buffer=buffer)
