...
def limit_contacts_by_rna_location_check_features_for_dna_contact(dgff3, contactfile, rnalocation, dnabuffer, rnabuffer, rnamatch, maxgapoverlap):
    import os
    from natsort import natsorted
    ...
    dnacigarcolumn = 9
    xscaff, xstart, xend = rnalocation  # rnalocation is list = [scaffold, start, stop], all integers

    outfeatures=[]  # the features where rna from rnalocation have 5' dna contacts inside features
    filteredcontacts=[]  # contact in filtered contacts have rna positions overlapping specified rnalocation coordinates
    with open(contactfile, 'r') as FILE:
        for count, line in enumerate(FILE):  # for each contact in contactfile
            ...
            if (rnascaff == str(xscaff)) and (rnaleft <= (xend + rnabuffer)) and (rnaright >= (xstart - rnabuffer)):
                ###
                ...
                for match in matches:
                    ...
                    matchesinsidefeature += len(range(max(xstart, match[0]), min(xend, match[1]) + 1))
                for gap in gaps:  # gap is list of len == 2, i.e. left and right coordinate of gap
                    gapsinsidefeature += len(range(max(xstart, gap[0]), min(xend, gap[1]) + 1))
                if matchesinsidefeature >= rnamatch:
                    matchgate = 1  # open gate, more then 10 bases match/mismatch feature
                if gapsinsidefeature / (xstart - xend + 1) > maxgapoverlap:
                    gapgate = 0  # close gate, gap covers more than 75% of feature. i.e. too many gaps

                if (matchgate == 1) and (gapgate == 1):
                    ###
                    filteredcontacts.append(line)
            if count % 1000000 == 0:
                print(f'Count: {count} | Current contact: {line.strip()}')

    print(f'Number of filtered contacts from rna location: {len(filteredcontacts)}')
    for count, line in enumerate(filteredcontacts):
        # now lets find if dna contact position aligns inside of a feature
        ...
        try:
            ...
            for k, v in dgff3[scaff].items():  # use gff3 index for scaffold to reduce runtime
                ...
                if (dna5prime >= (featurestart - dnabuffer)) and (dna5prime <= (featureend + dnabuffer)):
                    # if the 5' DNA position is inside feature then that means it is associated with chromatin
                    dcounts[k] = dcounts.setdefault(k, 0) + support
                    outlines.append(' '.join([k, line.strip()]))
                    featureids.append(k)
                    outfeatures.append('\t'.join([k, line.strip()] + v))
        if count % 100000 == 0:
            print(f'Count: {count} | Current contact: {line.strip()}')
    ...
    path, file = os.path.split(contactfile)
    # this file has all contacts inside features, and feature it maps within
    outfile = os.path.join(path, '.'.join(file.split('.')[:-1]) + f'.rnascaff{xscaff}start{xstart}end{xend}.dbuff{dnabuffer}.rbuff{rnabuffer}.match{rnamatch}.gap{maxgapoverlap}.features.tsv')
    with open(outfile, 'w') as OUT:
        OUT.write('\n'.join(outfeatures))
    # this file has all contacts inside features
    outfile = os.path.join(path, '.'.join(file.split('.')[:-1]) + f'.rnascaff{xscaff}start{xstart}end{xend}.dbuff{dnabuffer}.rbuff{rnabuffer}.match{rnamatch}.gap{maxgapoverlap}.txt')
    with open(outfile, 'w') as OUT:
        OUT.write('\n'.join(outlines))
    # number of lines in .counts.txt file = number of unique features with contacts inside
    outfilecounts = os.path.join(path, '.'.join(file.split('.')[:-1]) + f'.rnascaff{xscaff}start{xstart}end{xend}.dnabuff{dnabuffer}.rnabuff{rnabuffer}.match{rnamatch}.gap{maxgapoverlap}.counts.txt')
...
def main(forwardreadfile, reversereadfile, blastdatabase, aligndatabase, altreadfile1='/path/to/altreads1.fastq',
         ...
         'contacts in features', 'trans rna', 'get sequences', 'blast rna', 'cumulative distribution'], rnalen=20, dnalen=20,
         ...
         zoomscaffname='', zoomwinsize=20, scaffcoordmin=[0], scaffcoordmax=[0], features=['mRNA'], rnalocation=[]):
    ...
    # rnalocation is list = [scaffold, start, stop]
    ...
    # do not run this section 'trans rna' at the same time as 'contacts in features'
    if 'trans rna' in steps:
        if steps[0] == 'trans rna':
            contactfilenames = altreadfile1
        dgff3 = read_gff3_index_by_scaffold(annot_file, features)
        # rnalocation is list = [scaffold, start, stop]
        featurecontactfile = limit_contacts_by_rna_location_check_features_for_dna_contact(dgff3, contactfilenames, rnalocation, dnabuffer, rnabuffer, rnamatch, maxgapoverlap)
        dgff3 = None
