...
def limit_contacts_by_feature_with_indices(dgff3, contactfile, dnabuffer=0, rnabuffer=0):
    ...
    # i.e. rnascaffold, rnaleft, rnaright, rnaorientation, dnascaffold, dnaleft, dnaright, dnaorientation, contactstrength, readname1,readname2,readname3
    ...
    dnascaffcolumn = 4  # number use 0-based numbering for columns
    dnaleftcolumn = 5
    dnarightcolumn = 6
    dnaorientationcolumn = 7
                ...
                for k, v in dgff3[scaff].items():  # use gff3 index for scaffold to reduce runtime
                    ...
                    if (dna5prime >= (int(v[3]) - dnabuffer)) and (dna5prime <= (int(v[4]) + dnabuffer)):
                        ...
                        if rnaorientation == '+':
                            ...
                            if (rnascaff == scaff) and (rna3prime > int(v[3])) and (rna3prime <= (int(v[4]) + rnabuffer)):
                                dcounts[k] = dcounts.setdefault(k, 0) + support
                            ...
                            if (rnascaff == scaff) and (rna3prime >= (int(v[3]) - rnabuffer)) and (rna3prime < int(v[4])):
                                dcounts[k] = dcounts.setdefault(k, 0) + support
                                outlines.append(' '.join([k, line.strip()]))  # feature ID + contact information
                                featureids.append(k)
...
def convert_scaff_coords_to_continuous_genome_coords(contactfile, dgcoords):
    ...
    with open(contactfile, 'r') as FILE:
        for line in FILE:
            # rnascaffold rnaleft rnaright rnaorientation dnascaffold dnaleft dnaright dnaorientation AANNNAA strength
            l = line.strip().split()
            support = int(math.ceil(float(l[-1])))  # round up to nearest integer # math.ceil returns float()
            rnascaff = 'scaffold51_%d' % int(l[0][2:])
            dnascaff = 'scaffold51_%d' % int(l[4][2:])
            if l[3] == '+':  # if RNA is mapped in forward orientation
                rna3prime = int(l[2])
            elif l[3] == '-':  # if RNA read is mapped in reverse orientation
                rna3prime = int(l[1])
            if l[7] == '+':  # if DNA read is mapped in forward orientation
                dna5prime = int(l[5])
            elif l[7] == '-':  # if DNA read is mapped in reverse orientation
                dna5prime = int(l[6])
...
def summarize_raw_rna_dna_contacts_and_names(rawcontactfilenames):
    ...
    with open(rawcontactfilenames, 'r') as FILE:
        for l in FILE:  # if memory wasn't a problem i would do this
            # line structure rnascaffold, rnastartpos, rnaendpos, rnaorientation, dnascaff, dnastart, dnaend, dnaorientation, NNNbarcode, read name
    ...
    with open(outpathcircos, 'w') as OUT:
        ...
        OUT.write('\n'.join([' '.join(i.split()[:3] + i.split()[4:7] + ['thickness=' + str(math.log(len(d[i]) + 1, 2)) + 'p']) for i in natsorted(d)]))  # sortcontacts
            ...
            for rnapos in drna[k]:  # the read could map to multiple locations 'equally' well (according to MQ cutoff)
                ...
                for dnapos in ddna[k]:  # the read could map to multiple locations 'equally' well (according to MQ)
                    ...
                    if PCRDupRemoval is False:
                        ...
                        # rnascaff rnaleft rnaright rnaorientation dnascaff dnaleft dnaright dnaorientation readname
                        output.append([sntrim + rnapos[0].split('_')[1], rnapos[1], rnapos[2], rnapos[3],
                                       sntrim + dnapos[0].split('_')[1], dnapos[1], dnapos[2], dnapos[3],
                                       k])
        ...
        for line in FILE:
            ...
            if line[0] == '@':
                ...
                if f'{flag:012b}'[-3] == '0':  # if the read is mapped # '000000000100' == 4 == read unmapped #[-3] != 1
                    ...
                    rightcoord = get_rightmost_reference_based_alignment_coordinate(
                                        line.strip().split('\t')[5],
                                        leftcoord)
                    dreads.setdefault(readname, []).append([scaffold, str(leftcoord), str(rightcoord), orientation])
...
def main(forwardreadfile, reversereadfile, blastdatabase, aligndatabase, altreadfile1='/path/to/altreads1.fastq',
         ...
         'sequences in features', 'cumulative distribution'], rnalen=20, dnalen=20,
         rnamaxlen=1000, REsequence='GATC', windowsize=300, contactcutoff=1, MQ=1, dnabuffer=0, rnabuffer=0,
         sn='Paramecium tetraurelia', alignargs='',
    ...
    if 'contacts in features' in steps:
        ...
        featurecontactfile = limit_contacts_by_feature_with_indices(dgff3, contactfilenames, dnabuffer=dnabuffer, rnabuffer=rnabuffer)
    ...
    if 'get sequences' in steps:
        ...
        rnaseqfile, scnrnaseqfile = retrieve_rna_sequences_from_contacts_inside_features(seqnames, rnafastqfiles, dnabuffer=dnabuffer, rnabuffer=rnabuffer)

    if 'blast rna' in steps:
        if steps[0] == 'blast rna':
            rnaseqfile = altreadfile1
        run_blastn_match_db(rnaseqfile, blastdatabase, 6, 50.00, True)
