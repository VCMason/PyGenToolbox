...
def make_pandas_stacked_barplot(pivot_df, order, outpath):
    ...
    pivot_df.loc[:, order].plot.bar(stacked=True, figsize=(10, 7))  #  color=colors,
    plt.savefig(outpath)
    print('Output Stacked Barplot to:\n%s' % outpath)


def read_subtraction_by_mapping_single_end(data, d, dnames, aligndatabaselist, readfile, seqlength):
...
def main(fileextension='.fastq.gz', directory='', outplotname='MyOutputPlot.pdf', startseqlength=15, cleanreads=True,
    ...
    make_pandas_stacked_barplot(pivot_df, ref_prefixes, outpath)
