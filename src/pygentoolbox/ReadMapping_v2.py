...
def run_hisat2(aligndatabase, forwardfile, reversefile='', alignargs='', paired=True):
    import os
    import subprocess
    ...
    if paired == True:
        ...
        print('Starting Hisat2 paired end: aligning\n%s, %s' % (forwardfile, reversefile))
        outsamfile = os.path.join(path, 'hisat2', '.'.join(f.split('.')[:-2] + [referenceprefix, 'sam']))
        cmd = 'hisat2 -q %s -x %s -1 %s -2 %s -S %s' % (alignargs, aligndatabase, forwardfile, reversefile, outsamfile)
    elif paired == False:
        # then data is single end
        print('Starting Hisat2 single end: aligning\n%s' % forwardfile)
        outsamfile = os.path.join(path, 'hisat2', '.'.join(f.split('.')[:-2] + [referenceprefix, 'sam']))
        cmd = 'hisat2 -q %s -x %s -U %s -S %s' % (alignargs, aligndatabase, forwardfile, outsamfile)
    else:
        print('variable paired not specified')
...
def main(forward, reverse, directory='', alignargs='', skipsteps=[], paired=True,
    ...
    for count, forwardfile in enumerate(forwardlist):
        ...
        if 'fastp' not in skipsteps:
            ...
            if paired == True:
                ...
                forwardclean = run_fastp_single_end(forwardfile)
        ...
        if 'hisat2' not in skipsteps:
            ...
            samfile = run_hisat2(reference, forwardclean, reverseclean, alignargs, paired)
